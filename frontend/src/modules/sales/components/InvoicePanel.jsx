 ...
 * ¿Qué es? Modal con resumen de cuenta, descuentos y division
 * ¿Para qué? Facturar una mesa con reglas operativas configurables
 */

import { useEffect, useState } from 'react';
import { useQuery } from '@tanstack/react-query';
import { X, Receipt, Users, Printer, Percent, DollarSign, ShieldAlert } from 'lucide-react';
...
const InvoicePanel = ({ isOpen, onClose, onConfirm, table }) => {
  ...
  const handlePrint = () => {
    ...
    printWindow.document.write(`
      ...
      <body>
        ...
        <p style="text-align:center;font-size:12px;color:#666">Mesa ${table.tableNumber}</p>
        <hr/>
        ...
        ${discountAmount > 0 ? `<p style="text-align:right">Descuento: -${formatCurrency(discountAmount)}</p>` : ''}
        <div class="total" style="text-align:right">TOTAL: ${formatCurrency(finalTotal)}</div>
        ${splitCount > 1 ? `<p style="text-align:center;font-size:12px;color:#666;margin-top:8px">Dividido entre ${splitCount}: ${formatCurrency(perPerson)} c/u</p>` : ''}
  ...
  return (
    <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50" onClick={onClose}>
      <div
        className="w-full max-w-md max-h-[90vh] overflow-y-auto rounded-xl bg-white shadow-2xl m-4"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-center justify-between border-b px-6 py-4">
          <div className="flex items-center gap-2">
            <Receipt className="h-5 w-5 text-green-600" />
            <h2 className="text-lg font-bold text-gray-900">Facturar Mesa {table.tableNumber}</h2>
          </div>
          ...
          <div className="rounded-lg border border-gray-200 p-4">
            ...
            <div className="grid grid-cols-3 gap-2">
              <button
                ...
                className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${discountType === 'none' ? 'border-primary-300 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                Ninguno
              </button>
              <button
                onClick={() => setDiscountType('fixed')}
                className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${discountType === 'fixed' ? 'border-primary-300 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                Fijo
              </button>
              <button
                onClick={() => setDiscountType('percentage')}
                className={`rounded-lg border px-3 py-2 text-sm font-medium transition-colors ${discountType === 'percentage' ? 'border-primary-300 bg-primary-50 text-primary-700' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}
              >
                %
              </button>
            </div>
            ...
            {discountType !== 'none' && (
              <div className="mt-3 space-y-3">
                ...
                {requiresOverride && (
                  <label className="flex items-start gap-3 rounded-lg border border-yellow-200 bg-yellow-50 px-3 py-3">
                    <input
                      type="checkbox"
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-xs text-yellow-700">
                        Este descuento supera el umbral de {settings?.discountOverrideThresholdPercent}% definido en configuracion.
                      </p>
                    </div>
                  </label>
                )}
