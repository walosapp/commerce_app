...
DO $$
DECLARE
    v_password_hash TEXT := current_setting('walos.bootstrap_admin_password_hash', TRUE);
BEGIN
    IF v_password_hash IS NULL OR v_password_hash !~ '^\$2[aby]\$[0-9]{2}\$.{53}$' THEN
        RAISE EXCEPTION 'walos.bootstrap_admin_password_hash debe contener un hash BCrypt suministrado por el operador';
    END IF;
END $$;
...
-- Si la migracion 019 ya existe, materializar sus defaults para todo comercio
-- creado por este seed. SQL dinamico mantiene compatibilidad con instalaciones
-- antiguas donde las tablas de features aun no existen.
DO $$
BEGIN
...
DECLARE
    ...
    v_role_id BIGINT;
    v_branch_id BIGINT;
    v_password_hash TEXT := current_setting('walos.bootstrap_admin_password_hash');
BEGIN
    SELECT id INTO v_company_id FROM core.companies WHERE tax_id = 'DEMO123456789';
...
SELECT set_config('walos.bootstrap_admin_password_hash', '', FALSE);
COMMIT;
